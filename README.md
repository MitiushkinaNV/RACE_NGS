...
GenotypeSpliceVariants.py

This script allows to numerically count reads, which contain sequences aligning (at least partially) with the region specified in the input file with genomic coordinates (the example of such file is provided) and/or lacking other sequences specified in the same file. The required arguments are -f/--file_in (input bam file), -s/--sample_name (sample name), -c/--coordinates (input file with coordinates), -q/--filter_file (file with query names for each primer generated by Filter_RACEbam). The output .tsv file will contain read counts for each allele, specified in the coordinates file, along with the percent of reads belonging to each of alleles of the specified variant.

If the program is called multiple times (one time for each sample), a single file with the default name "SpliceVariants_results.tsv" will be created, containing results for all tested samples.
